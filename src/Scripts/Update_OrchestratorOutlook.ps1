...
ID: Update_OrchestratorOutlook
TITLE: KORSiOS - [Windows Update]• Bloat • Empêcher Windows Update Orchestrator d'installer Outlook
...
Notes:
- Backup/rollback stocké dans %LOCALAPPDATA%\KORSiOS\TweakBackups\Update_OrchestratorOutlook
...
$TweakId = "Update_OrchestratorOutlook"
$TweakTitle = "KORSiOS - [Windows Update]• Bloat • Empêcher Windows Update Orchestrator d'installer Outlook"
...
function Save-RegistryBackup([string[]]$regKeys, [string]$backupDir) {
    ...
    # Export clé par clé dans des fichiers temporaires puis concat en un seul .reg (plus robuste)
    ...
    foreach ($k in $regKeys) {
        ...
        # reg.exe export retourne code !=0 si la clé n'existe pas : on ignore (c'est ok)
    ...
    # Concat: header + blocs (en sautant le header redondant)
    ...
    foreach ($f in $exported) {
        ...
        # Supprime la première ligne "Windows Registry Editor Version 5.00" si présente
...
function Import-RegFromString([string]$regText) {
    $tmp = Join-Path $env:TEMP ("Update_OrchestratorOutlook_" + [Guid]::NewGuid().ToString("N") + ".reg")
    # reg.exe import attend UTF-16LE (Unicode) idéalement
...
function Apply-Tweak {
    $backupDir = Join-Path $env:LOCALAPPDATA ("KORSiOS\TweakBackups\Update_OrchestratorOutlook")
    $regKeys = @(
    ...
    if (-not (Test-IsAdmin)) {
        # Certains tweaks (HKLM/HKCR) nécessitent admin: on informe mais on tente quand même (HKCU passera).
...
; KORSiOS - [Windows Update]• Bloat • Empêcher Windows Update Orchestrator d'installer Outlook
; ID: Update_OrchestratorOutlook
; DESC: Empêche l’installation automatique de Microsoft Outlook lors de l’OOBE ou via Windows Update.
; IMPACT: - Supprime la tâche OutlookUpdate liée à l’OOBE (première configuration).
; IMPACT: - Marque la tâche OutlookUpdate comme déjà exécutée dans l’orchestrateur Windows Update.
; IMPACT: - Bloque le téléchargement et l’installation automatique de la nouvelle application Outlook.
; RISK: - Absence de la nouvelle app Outlook.
; RISK: - Aucun impact sur la stabilité, la sécurité ou les mises à jour de sécurité classiques.
; NOTE: Réglage anti-bloat / OOBE maîtrisé : évite l’installation forcée d’Outlook lors de la première utilisation ou après une mise à niveau.
; RESTART: REBOOT
[-HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\WindowsUpdate\Orchestrator\UScheduler_Oobe\OutlookUpdate]
[HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Orchestrator\UScheduler\OutlookUpdate]
...
function Status-Tweak {
    # Best-effort: vérifie si au moins une clé du tweak existe (ou une valeur typique)
    $regKeys = @(
        "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\WindowsUpdate\Orchestrator\UScheduler_Oobe\OutlookUpdate"
        "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Orchestrator\UScheduler\OutlookUpdate"
    )
